...
3. execute `docker compose up --build --wait` to turn on the containers.
4. Run the command `docker ps` and make sure that 2 new containers with elastic in their name exist.
...
5. Now, you can go to the firefox and run: `localhost:5601`
6. You might need to enter username and password:
   * Username: elastic
   * Password: saved in .env file in the elastic directory (from step 1)
7. Go to the burger sign in the top left of the site and clicke on Discover.
8. In the DataView tab click on `scanner` to view the logs collected by running the scanner.
